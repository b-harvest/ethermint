...
func (suite *BaseTestSuite) MintFeeCollectorVirtual(coins sdk.Coins) {
	...
	addVirtualCoins(
		suite.Ctx.TransientStore(suite.App.GetStoreKey(banktypes.TStoreKey)),
		suite.Ctx.TxIndex(),
...
// addVirtualCoins mocks actual logic of bank keeper's private method
func addVirtualCoins(tstore storetypes.KVStore, txIndex int, addr sdk.AccAddress, amt sdk.Coins) {
	...
	var coins sdk.Coins
	var err error
	value := tstore.Get(key)
	if value != nil {
		coins, err = sdk.ParseCoinsNormalized(string(value))
		if err != nil {
			panic(err)
		}
	}
	coins = coins.Add(amt...)
	tstore.Set(key, []byte(coins.String()))
}
