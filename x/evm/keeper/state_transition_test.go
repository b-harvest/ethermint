...
func (suite *StateTransitionTestSuite) SetupTest() {
	...
	t := suite.T()
	suite.SetupTestWithCb(t, func(a *app.EthermintApp, genesis testutil.GenesisState) testutil.GenesisState {
...
func (suite *StateTransitionTestSuite) TestGetHashFn() {
	...
	testCases := []struct {
		...
		},
		{
			...
			1,
			func() {
				suite.Require().NoError(suite.App.StakingKeeper.SetHistoricalInfo(suite.Ctx, 1, &stakingtypes.HistoricalInfo{}))
				suite.Ctx = suite.Ctx.WithBlockHeight(10)
			},
			common.Hash{},
		},
		{
			...
			func() {
				...
				suite.Require().NoError(suite.App.StakingKeeper.SetHistoricalInfo(suite.Ctx, 1, histInfo))
				suite.Ctx = suite.Ctx.WithBlockHeight(10)
			},
			common.BytesToHash(hash),
		},
...
func (suite *StateTransitionTestSuite) TestGetCoinbaseAddress() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			coinbase, err := suite.App.EvmKeeper.GetCoinbaseAddress(suite.Ctx, proposerAddress)
			if tc.expPass {
				suite.Require().NoError(err)
		...
		},
		{
			...
			malleate: func() {
				...
				m, err = suite.createContractGethMsg(
					...
					params.GetChainConfig().EthereumConfig(suite.App.EvmKeeper.ChainID()),
					big.NewInt(-100),
				)
				suite.Require().NoError(err)
			},
...
func (suite *StateTransitionTestSuite) TestApplyMessage() {
	...
	proposerAddress := suite.Ctx.BlockHeader().ProposerAddress
	_, err := suite.App.EvmKeeper.EVMConfig(suite.Ctx, proposerAddress, big.NewInt(9000))
	suite.Require().NoError(err)
