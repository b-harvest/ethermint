...
func NewWithParams(ctx sdk.Context, keeper Keeper, txConfig TxConfig, evmDenom string) *StateDB {
	...
	if parentCacheMS, ok := ctx.MultiStore().(cachemulti.Store); ok {
		cacheMS = parentCacheMS.CacheMultiStore()
		commitMS = append(commitMS, cacheMS.Write)
	} else {
...
func (s *StateDB) SetState(addr common.Address, key, value common.Hash) {
	stateObject := s.getOrNewStateObject(addr) // TODO(zsystm): nil check?
...
func (s *StateDB) Commit() error {
	...
	ei := len(s.commitMS) - 1
	for si := ei; si >= 0; si-- {
		// commit the native state
		// child store should be committed first
		s.commitMS[si]()
	}
