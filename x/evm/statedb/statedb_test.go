...
func (suite *StateDBTestSuite) TestAccount() {
	...
	testCases := []struct {
		...
		{"empty account", func(db *statedb.StateDB, cms storetypes.MultiStore) {
			...
			db, err := statedb.New(ctx, keeper, txConfig)
...
func (suite *StateDBTestSuite) TestAccountOverride() {
	...
	db, err := statedb.New(ctx, keeper, emptyTxConfig)
	suite.Require().NoError(err)
...
func (suite *StateDBTestSuite) TestNativeAction() {
	...
	stateDB, err := statedb.New(ctx, keeper, emptyTxConfig)
	suite.Require().NoError(err)
	...
	suite.Require().Equal(sdk.Events{{Type: "success1"}}, ctx.EventManager().Events())
}
...
func cloneRawState(_ *testing.T, cms storetypes.MultiStore) map[string]map[string][]byte {
...
func newTestKeeper(_ *testing.T, cms storetypes.MultiStore) (sdk.Context, *evmkeeper.Keeper) {
	...
	evmKeeper := evmkeeper.NewKeeper(
		appCodec,
		runtime.NewKVStoreService(testStoreKeys[banktypes.StoreKey]),
		testStoreKeys[evmtypes.StoreKey],
