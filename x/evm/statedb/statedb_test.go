...
func (suite *StateDBTestSuite) TestRevertSnapshot() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			rev := db.Snapshot()
			tc.malleate(db)
			//afterMal := cloneRawState(suite.T(), raw)
			//suite.T().Log("after malleate", afterMal)
...
func (suite *StateDBTestSuite) TestNativeAction() {
	...
	tStoreKey := testTStoreKeys[evmtypes.TransientKey]
	...
	tk, tv, mk, mv := []byte("transient"), []byte("value"), []byte("mem"), []byte("value")
	stateDB.ExecuteNativeAction(contract, eventConverter, func(ctx sdk.Context) error {
		...
		tStore := ctx.TransientStore(tStoreKey)
		tStore.Set(tk, tv)
	...
	stateDB.ExecuteNativeAction(contract, eventConverter, func(ctx sdk.Context) error {
		...
		tStore := ctx.TransientStore(tStoreKey)
		suite.Require().Equal(tv, tStore.Get(tk))
	...
	fv := ctx.KVStore(storeKey).Get([]byte("failure1"))
	suite.Require().Nil(fv)

	// check events
	suite.Require().Equal(sdk.Events{{Type: "success1"}}, stateDB.NativeEvents())
	logs := stateDB.Logs()
	suite.Equal(1, len(logs))
	suite.EqualValues(contract, logs[0].Address)
	suite.Equal([]byte("success1"), logs[0].Data)
	...
	logs = stateDB.Logs()
	suite.Equal(2, len(logs))
	suite.EqualValues(contract, logs[0].Address)
	suite.Equal([]byte("success1"), logs[0].Data)
	suite.EqualValues(contract, logs[1].Address)
	suite.Equal([]byte("success2"), logs[1].Data)
	// test query
	stateDB.ExecuteNativeAction(contract, nil, func(ctx sdk.Context) error {
		store := ctx.KVStore(storeKey)
		suite.Require().Equal([]byte("value"), store.Get([]byte("success1")))
...
var (
	...
	testTStoreKeys = storetypes.NewTransientStoreKeys(banktypes.TStoreKey, evmtypes.TransientKey)
...
func newTestKeeper(t *testing.T, cms storetypes.MultiStore) (sdk.Context, *evmkeeper.Keeper) {
	...
	bankKeeper := bankkeeper.NewBaseKeeper(
		...
		authAddr,
		log.NewNopLogger(),
		testTStoreKeys[banktypes.TStoreKey],
	)
	evmKeeper := evmkeeper.NewKeeper(
		appCodec,
		runtime.NewKVStoreService(testStoreKeys[evmtypes.StoreKey]),
		testTStoreKeys[evmtypes.TransientKey],
...
func setupTestEnv(t *testing.T) (storetypes.MultiStore, sdk.Context, *evmkeeper.Keeper) {
	...
	for _, key := range testTStoreKeys {
